		...
		var _process = function(request, response) {
			...
			this.listen('response', function(request, response) {
				try {
					...
					response.end(response.message+'', response.encoding);
		...
		var _stream = function(request) { 
			...
			events.start = function() {
				...
				form.onPart = function(part) {
					...
					files.push({ key: part.name, name: part.filename, mime: part.mime });
					
					this.emit('file-start', part.filename, part.mime, part.name);
