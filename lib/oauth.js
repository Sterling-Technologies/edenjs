module.exports = function($) {
	return this.define(function(public) {
		...
		public.__construct = function(url, key, secret) {
			...
			this.url	= url;
			this.time 	= $.load('time').time();
			this.nonce 	= $.load('string', $.uid()).md5().get();
		...
		public.getHmacSha1Signature = function(query) {
			...
			query = $.load('hash', params).concat(query).get(); //merge the params and the query
			...
			return $.load('string', string).hmac(key, 'sha1').get();
		};
		...
		public.send = function(query, callback) {
			...
			if(this.authorization) {
				...
				query = $.load('hash', authorization).concat(query).get();
		...
		var _buildQuery = function(query, separator, noQuotes, subList) {
			...
			query.each(function(key, value) {
				if($.get('array').isArray(value)) {
					value = $.load('array', value).natsort().get();
		...
		var _encode = function(value) {
			if($.get('hash').isHash(value) 
			|| $.get('array').isArray(value)) {
			...
			return $.load('string', ''+value).urlEncode().get();
		};
		...
		var _parseString = function(value) {
			...
			var keyValue = value.split('&').each(function(key, pair) {
